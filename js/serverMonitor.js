$(function () {
    ...
    connection.onmessage = function (message) {
        ...
        if (json.type === 'message') {
             ...
             * CPU Load HTML
             */
            $('#cpuLoad').html('').append($('<span>').addClass('statusMain').text(json.data[0].now)).append($('<span>').addClass('statusSecondary').text('/' + json.data[0].count)).append($('<div>').addClass('pull-left').html(json.data[0].five + " <br> 5 Mins")).append($('<div>').addClass('pull-right').html(json.data[0].fifteen + " <br> 15 Mins"));
            /**
             * Memory Usage HTML
             */
            ...
            $.each(json.data[0].drives, function (key, value) {
                if (typeof value.mount !== "undefined") {
                    $('#driveList').find('tbody').append($('<tr>').append($('<td>').html($.trim(value.mount)).width('40%').addClass("driveMount")).append($('<td>').append($('<div>').addClass('progress progress-info progress-striped').append($('<div>').addClass('bar').width(value.percent)))));
                }
            });
            ...
            $.each(json.data[0].ports, function (key, value) {
                ...
                $('#portScan').find('tbody').append($('<tr>').append($('<td>').html(value[0])).append($('<td>').html(response)));
            });
            if (isFirst === true) {
                grid.slideDown('slow');
                isFirst = false;
